...
(defcommand
  ...
  (let [response (client/get tokeninfo-url {:oauth-token      access-token
                                            :as               :json-string-keys
...
(defn make-cached-access-token-resolver [tokeninfo-url {:keys [ttl-ms max-size] :or {ttl-ms 120000 max-size 100}}]
  (memo/fifo #(resolve-access-token tokeninfo-url %)
